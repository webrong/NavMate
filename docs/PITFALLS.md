...
## P15. 流式升级必须 ignore_user_abort，否则客户端断开 = 升级半途暴毙（2026-09-25）

**现象**：给升级 SSE 加了下载进度事件后，用户点「取消」或超时断开，后端升级在下一次 flush 时被 PHP 直接终止——catch 清理（恢复备份/关维护模式/记日志）全部跳过，**站点永久停在维护模式 503**。没加进度事件之前反而没事：下载阶段无输出，PHP 根本感知不到断开。

**根因**：PHP 默认 `ignore_user_abort=Off`，脚本在下一次输出时检测到连接中断就自杀。StreamedResponse + 定期 flush = 每个事件都是一个断开检测点。

**修复**：SSE 端点闭包开头 `ignore_user_abort(true)`（断开不杀进程），取消改为**优雅中止**——每个进度点检查 `connection_aborted()`，非零则抛异常走进 doUpdate 的 catch 完整清理路径。

**教训**：给长耗时流式端点加任何"输出"之前，先想清楚客户端断开后脚本该死该活。死 = 清理不执行；活 = 需要 `ignore_user_abort` + 主动的 `connection_aborted()` 检查点。

## P16. 路径安全校验别用 ASCII 字符集白名单——中文文件名是合法乘客（2026-09-25）

**现象**：升级解压的白名单 `^[A-Za-z0-9_.\-/]+$` 让每次升级在「解压替换」步必然抛"压缩包包含非法路径"，且 restoreBackup 同款白名单让自动回滚也失效（双重死亡）。仓库里真实存在 30 个 git 跟踪的中文文件名文件（`public/static/image/QQ图片*.png` 等），会打进 release zip。用 `git ls-files` 查它们时注意默认 `core.quotepath` 会把非 ASCII 转成八进制转义，得加 `-c core.quotepath=false` 才看得见。

**修复**：字符集白名单改为**结构检查**——只拒绝 `..` 段、反斜杠、`/` 开头、盘符 `^[A-Za-z]:`、NUL 字节；UTF-8 文件名放行。

**教训**：安全校验的原则是"拒绝已知危险结构"，不是"只放行我见过的形态"。白名单字符集对「文件名来自不受控的现实世界」的场景（用户上传、代码库内容）天然不成立。
