<template>
  <a :href="site.url" target="_blank" rel="noopener noreferrer" class="site-card" :aria-label="'访问 ' + site.title" @click="trackClick">
    <button
      v-if="authStore.isAuthenticated"
      class="site-fav"
      :class="{ active: isFav }"
      @click.stop.prevent="handleFav"
      :aria-label="isFav ? '取消收藏 ' + site.title : '收藏 ' + site.title"
      :title="isFav ? '取消收藏' : '收藏'"
    >
    ...
    <div v-if="site.favicon_url && !imgError" class="site-card-bg" :style="{ backgroundImage: `url(${site.favicon_url})` }">
    </div>
...
// Cache isFavorite as a single computed per card (was called 3x in template)
