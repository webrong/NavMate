...
// Bits 0-3: leap month (0=none) | Bit 4: leap month 30(1)/29(0) | Bits 5-16: months 12..1 size 30(1)/29(0)
const D = [
...
function leapMonth(y) { return D[y - 1900] & 0xf; }

function leapDays(y) {
  return leapMonth(y) ? ((D[y - 1900] & 0x10000) ? 30 : 29) : 0;
}

function monthDays(y, m) {
  return (D[y - 1900] & (0x10000 >> m)) ? 30 : 29;
}
...
function solarToLunar(year, month, day) {
  ...
  let lYear;
  for (lYear = 1900; lYear < 2101; lYear++) {
    const days = yearDays(lYear);
  ...
  const months = [];
  for (let m = 1; m <= 12; m++) {
    months.push({ month: m, days: monthDays(lYear, m), isLeap: false });
    if (m === leap) {
      months.push({ month: m, days: leapDays(lYear), isLeap: true });
    }
  }

  let lMonth = 1, lDay = 1, isLeap = false;
  for (let i = 0; i < months.length; i++) {
    if (offset < months[i].days) {
      lMonth = months[i].month;
...
const MonthN = ['正','二','三','四','五','六','七','八','九','十','冬','腊'];
const DayN = [
